...
type ResponseFut =
    Pin<Box<dyn Future<Output = Result<Response, reqwest_middleware::Error>> + Send>>;
enum ResponseState {
    Pending(ResponseFut),
    Ready(Response),
    None,
...
impl Stream for ReqwestStream {
    type Item = Result<Bytes, reqwest_middleware::Error>;
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        match &mut self.resp {
            ...
            ResponseState::Ready(resp) => {
                if let Err(e) = resp.error_for_status_ref() {
                    self.resp = ResponseState::None;
                    return Poll::Ready(Some(Err(reqwest_middleware::Error::Reqwest(e))));
                }
        ...
        match chunk_global {
            ...
            Err(e) => {
                self.resp = ResponseState::None;
                Poll::Ready(Some(Err(reqwest_middleware::Error::Reqwest(e))))
            }
        }
    }
}
