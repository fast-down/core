...
pub fn to_string(progress: &[ProgressEntry]) -> String {
    progress
        .iter()
...
pub fn from_str(text: &str) -> Vec<ProgressEntry> {
    if text.is_empty() {
        return vec![];
    }
    text.split(',')
...
mod tests {
    ...
    fn test_str_to_progress() {
        assert_eq!(from_str("1-10,20-30,40-50"), vec![1..11, 20..31, 40..51]);
        assert_eq!(from_str(""), vec![] as Vec<ProgressEntry>);
    ...
    fn test_display_progress_empty() {
        let progress: Vec<ProgressEntry> = vec![];
