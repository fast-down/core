...
pub mod download_file {
    ...
    impl DownloadResult {
        pub(crate) fn from_single(value: (crossbeam_channel::Receiver<Event>, JoinHandle<()>)) -> Self {
        ...
        pub(crate) fn from_multi(value: (crossbeam_channel::Receiver<Event>, Vec<JoinHandle<()>>)) -> Self {
    ...
    pub fn download(
        ...
        {
            Ok(DownloadResult::from_single(single::download(options.url, FileWriter::new::<4>(file)?, single::DownloadOptions {
                client: options.client,
                get_chunk_size: options.get_chunk_size,
                retry_gap: options.retry_gap,
            })?))
        } else {
            Ok(DownloadResult::from_multi(multi::download(options.url, FileWriter::new::<4>(file)?, multi::DownloadOptions {
                client: options.client,
